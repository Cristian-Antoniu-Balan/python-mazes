...
class Grid:
    ...
    def generateImg(self, cell_size=defaults.cell_size):
        ...
        for cell in self.each_cell():
            ...
            if cell.north is None:
                ImageDraw.Draw(img).line((x1, y1, x2, y1), wall_color, 2 * half_wall_width)
            if cell.west is None:
                ImageDraw.Draw(img).line((x1, y1, x1, y2), wall_color, 2 * half_wall_width)

            if not cell.is_linked(cell.east):
                ImageDraw.Draw(img).line((x2, y1, x2, y2), wall_color, 2 * half_wall_width)
            if not cell.is_linked(cell.south):
                ImageDraw.Draw(img).line((x1, y2, x2, y2), wall_color, 2 * half_wall_width)
    ...
    def generateSvg(self, cell_size=defaults.cell_size):
        ...
        for cell in self.each_cell():
            ...
            if cell.north is None:
                data.append(line(x1, y1, x2, y1))
            if cell.west is None:
                data.append(line(x1, y1, x1, y2))

            if not cell.is_linked(cell.east):
                data.append(line(x2, y1, x2, y2))
            if not cell.is_linked(cell.south):
                data.append(line(x1, y2, x2, y2))
