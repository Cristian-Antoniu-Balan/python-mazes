...
class PolarGrid(DistanceGrid):
    ...
    #         ImageDraw.Draw(img).arc(inner_arc_bounding_box, math.degrees(
    #             theta_ccw), math.degrees(theta_cw), color, 2 * half_wall_width)
    #         ImageDraw.Draw(img).arc(outer_arc_bounding_box, math.degrees(
    #             theta_ccw), math.degrees(theta_cw), color, 2 * half_wall_width)
    ...
    #         [ax, ay, bx, by, cx, cy, dx, dy, xm, ym, inner_radius, outer_radius, theta_ccw,
    #             theta_cw, theta] = self.cell_coords(cell, cell_size, 0, width / 2)
    ...
    #             ImageDraw.Draw(img).arc(inner_arc_bounding_box, math.degrees(theta_ccw),
    #                                     math.degrees(theta_cw), wall_color, 2 * half_wall_width)
    ...
    #                     drawCell(ax, ay, bx, by, cx, cy, dx, dy, xm, ym, theta_ccw,
    #                              theta_cw, color='RGB(255, 0, 0)', fill_color=(255, 0, 0))
    ...
    def generateSvg(self, cell_size=defaults.cell_size):
        ...
        fill_flood_base_color = (0, 0, 255)

        total_height = cell_size * self.rows + 2 * half_wall_width
        total_width = total_height

        def color(intensity=0):
            return (fill_flood_base_color + (intensity,))
        ...
        def cell_filled(ax=0, ay=0, bx=0, by=0, cx=0, cy=0, dx=0, dy=0, inner_radius=0, outer_radius=0, width=2 * half_wall_width, color=wall_color, fill_color=wall_color):
            return f"""<path d=\"
                            ...
                            Z\"
                    fill=\"RGBA{fill_color}\" stroke-width=\"{width}\" stroke=\"RGB{color}\" />"""

        def cell_center_filled(cx=0, cy=0, r=0, width=2 * half_wall_width, color=wall_color, fill_color=wall_color):
            return f"<circle cx=\"{cx}\" cy=\"{cy}\" r=\"{r}\" fill=\"RGBA{fill_color}\" stroke-width=\"{width}\" stroke=\"RGB{color}\"/>"
        ...
        for cell in self.each_cell():
            ...
            [ax, ay, bx, by, cx, cy, dx, dy, xm, ym, inner_radius, outer_radius, theta_ccw,
                theta_cw, theta] = self.cell_coords(cell, cell_size, 0, total_width / 2)

            if not cell.is_linked(cell.inward):
                maze.append(arc(ax, ay, inner_radius, inner_radius, cx, cy))
            if not cell.is_linked(cell.cw):
                maze.append(line(ax, ay, bx, by))

            if (cell.path or cell.end_path):
                group = ""
                if cell.path:
                    group = "regular"
                    fill_color = fill_color_path
                else:
                    group = "end"
                    fill_color = fill_color_end_path

                layer_path.append(cell_text(xm, ym, self.contents_of(cell)[constants.MODE_DISTANCE]))

                if cell.row == 0:
                    layer_color[group].append(cell_center_filled(xm, ym, outer_radius, 0, fill_color=fill_color))
                    continue
                else:
                    layer_color[group].append(cell_filled(ax, ay, bx, by, cx, cy, dx, dy,
                                                          inner_radius, outer_radius, width=0, fill_color=fill_color))

            if cell.row == 0:
                layer_flood.append(cell_center_filled(xm, ym, outer_radius, 0,
                                                      fill_color=color(self.contents_of(cell)[constants.MODE_FLOOD])))
            else:
                layer_flood.append(cell_filled(ax, ay, bx, by, cx, cy, dx, dy,
                                               inner_radius, outer_radius, width=0, fill_color=color(self.contents_of(cell)[constants.MODE_FLOOD])))
            # match self.mode:
            #     case constants.MODE_DISTANCE | constants.MODE_PATH:
            ...
            #             if cell.row == 0:
            #                 layer_path.append(cell_center_filled(xm, ym, outer_radius, 0, fill_color=fill_color))
            #                 continue
            #             else:
            #                 layer_path.append(cell_filled(ax, ay, bx, by, cx, cy, dx, dy,
            #                                               inner_radius, outer_radius, width=0, fill_color=fill_color))
            #     case constants.MODE_COLOR:
            #         if cell.row == 0:
            #             layer_flood.append(cell_center_filled(xm, ym, outer_radius, 0,
            #                                fill_color=color(self.contents_of(cell))))
            #             continue
            #         else:
            #             layer_flood.append(cell_filled(ax, ay, bx, by, cx, cy, dx, dy,
            #                                            inner_radius, outer_radius, width=0, fill_color=color(self.contents_of(cell))))
            #     #     ImageDraw.Draw(img).rectangle((x1, y1, x2, y2),color(self.contents_of(cell)))
            #     #     # define a better way to find start and end cells
            #     #     if self.contents_of(cell) == 0:
            #     #         ImageDraw.Draw(img).text((xm, ym), '0', 'red', font, 'mm', 1, 'center')
        ...
        maze_with_color = layer_color["regular"] + layer_color["end"] + maze
        maze_with_color_and_path = maze_with_color + layer_path
        maze_with_flood = layer_flood + maze
        mazes = []
        for data in [maze, maze_with_color, maze_with_color_and_path, maze_with_flood]:
            file = header + data
            file.append(f"</svg>")
            mazes.append("\n".join(file))
